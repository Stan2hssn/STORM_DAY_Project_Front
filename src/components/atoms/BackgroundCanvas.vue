...
const palettes = {
  light: {
    ...
    b: new Vector3(0.20, 0.25, 0.20),
    c: new Vector3(0.6, 0.5, 0.3),
    d: new Vector3(0.0, 0.08, 0.3),
    bg: new Vector3(0.93, 0.91, 0.90),
    // ramp: rose → coral → peach → gold → pale
    ramp: [
      new Vector3(0.85, 0.30, 0.35),
      new Vector3(0.92, 0.45, 0.30),
      new Vector3(0.95, 0.65, 0.40),
      new Vector3(0.97, 0.82, 0.55),
      new Vector3(0.98, 0.94, 0.90),
    ],
  },
  dark: {
    a: new Vector3(0.35, 0.41, 0.25),
    b: new Vector3(0.10, 0.10, 0.08),
    c: new Vector3(0.5, 0.4, 0.3),
    d: new Vector3(0.0, 0.1, 0.15),
    bg: new Vector3(0.05, 0.08, 0.06),
    // ramp: sage → light olive → pale green → mint → near-white
    ramp: [
      new Vector3(0.35, 0.41, 0.25),  // #5A6841 olive
      new Vector3(0.50, 0.55, 0.38),  // medium sage
      new Vector3(0.65, 0.68, 0.55),  // light sage
      new Vector3(0.80, 0.82, 0.72),  // pale green
...
function loadGrainTexture() {
  ...
  const tex = loader.load('/textures/noise_1.png')
  tex.wrapS = RepeatWrapping
  tex.wrapT = RepeatWrapping
  return tex
}

// ── Tweakpane params (mutated live by the GUI) ──
const params = {
...
function setupTweakpane() {
  pane = new Pane({ title: 'Background Shader' })

  const fNoise = pane.addFolder({ title: 'Noise' })
  fNoise.addBinding(params, 'noiseScale', { min: 0.1, max: 5, step: 0.1 })
  fNoise.addBinding(params, 'noiseSpeed', { min: 0.01, max: 0.5, step: 0.01 })
  fNoise.addBinding(params, 'timeScale', { min: 0.01, max: 1, step: 0.01, label: 'time scale' })
  fNoise.addBinding(params, 'grainIntensity', { min: 0, max: 0.3, step: 0.005 })
  fNoise.addBinding(params, 'grainScale', { min: 0.1, max: 50, step: 0.1, label: 'grain scale' })
  fNoise.addBinding(params, 'fadeMix', { min: 0, max: 0.8, step: 0.01, label: 'fade mix' })

  const fBlobs = pane.addFolder({ title: 'Blobs' })
  fBlobs.addBinding(params, 'blobRadius1', { min: 0.05, max: 1, step: 0.01, label: 'radius 1' })
  fBlobs.addBinding(params, 'blobRadius2', { min: 0.05, max: 1, step: 0.01, label: 'radius 2' })
  fBlobs.addBinding(params, 'blobWarp1', { min: 0, max: 0.4, step: 0.01, label: 'warp 1' })
  fBlobs.addBinding(params, 'blobWarp2', { min: 0, max: 0.4, step: 0.01, label: 'warp 2' })

  const fPalette = pane.addFolder({ title: 'Palette (IQ)' })
  fPalette.addBinding(params, 'paletteA', {
    x: { min: 0, max: 1, step: 0.01 },
    y: { min: 0, max: 1, step: 0.01 },
    z: { min: 0, max: 1, step: 0.01 },
    label: 'A (base)',
  })
  fPalette.addBinding(params, 'paletteB', {
    x: { min: 0, max: 1, step: 0.01 },
    y: { min: 0, max: 1, step: 0.01 },
    z: { min: 0, max: 1, step: 0.01 },
    label: 'B (amp)',
  })
  fPalette.addBinding(params, 'paletteC', {
    x: { min: 0, max: 2, step: 0.01 },
    y: { min: 0, max: 2, step: 0.01 },
    z: { min: 0, max: 2, step: 0.01 },
    label: 'C (freq)',
  })
  fPalette.addBinding(params, 'paletteD', {
    x: { min: 0, max: 1, step: 0.01 },
    y: { min: 0, max: 1, step: 0.01 },
    z: { min: 0, max: 1, step: 0.01 },
    label: 'D (phase)',
  })
  fPalette.addBinding(params, 'bgColor', {
    x: { min: 0, max: 1, step: 0.01 },
    y: { min: 0, max: 1, step: 0.01 },
    z: { min: 0, max: 1, step: 0.01 },
    label: 'Background',
  ...
  fDebug.addBinding(params, 'debug', {
    view: 'list',
    label: 'view',
    options: [
      { text: 'Off', value: 0 },
      { text: 'Blob 1', value: 1 },
      { text: 'Blob 2', value: 2 },
      { text: 'Both (R/G)', value: 3 },
      { text: 'UV Warp', value: 4 },
      { text: 'Grain', value: 5 },
    ],
  })

  pane.addButton({ title: 'Log values to console' }).on('click', () => {
    console.log(JSON.stringify(params, null, 2))
  })

  pane.on('change', syncUniforms)
}
...
function createScene(canvas: HTMLCanvasElement) {
  ...
  const grainTex = loadGrainTexture()
  ...
  function tick() {
    ...
    // Lerp palette params towards target
    lerpParamsToTarget(dt)
    syncUniforms()

    material.uniforms.uTime!.value = (now - startTime) * 0.005
