...
void main() {
  ...
  // ── Warp UVs (organic blob shape) ──
  vec2 warpedPos = pos + vec2(
    snoise(vec3(pos * uNoiseScale, t * 0.8)) * uBlobWarp1,
    snoise(vec3(pos * uNoiseScale + 5.0, t * 0.6)) * uBlobWarp2
  );
  ...
  float b1 = 1.0 - smoothstep(0.0, uBlobRadius1, d1);
  float b2 = 1.0 - smoothstep(0.0, uBlobRadius2, d2);
  ...
  col += (grain - 0.5) * uGrainIntensity;

  // ── Debug ──
  if (uDebug == 1) { gl_FragColor = vec4(vec3(b1), 1.0); return; }
  if (uDebug == 2) { gl_FragColor = vec4(vec3(b2), 1.0); return; }
  if (uDebug == 3) { gl_FragColor = vec4(b1, b2, 0.0, 1.0); return; }
  if (uDebug == 4) {
    vec2 wd = warpedPos - pos;
    gl_FragColor = vec4(wd * 5.0 + 0.5, 0.5, 1.0); return;
  }
  if (uDebug == 5) { gl_FragColor = vec4(vec3(grain), 1.0); return; }
